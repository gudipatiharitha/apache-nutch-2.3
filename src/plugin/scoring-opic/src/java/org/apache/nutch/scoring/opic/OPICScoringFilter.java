...
public class OPICScoringFilter implements ScoringFilter {
  ...
  public void injectedScore(String url, WebPage row)
	  ...
	  System.out.println("insdide injectedscore");
  ...
  public void initialScore(String url, WebPage row)
      throws ScoringFilterException {
	  System.out.println("insdide initialscore");
    row.setScore(0.0f);
  ...
  public float generatorSortValue(String url, WebPage row, float initSort)
      throws ScoringFilterException {
	  System.out.println("insdide generatesort");
  ...
  public void updateScore(String url, WebPage row,
    ...
    float adjust = 0.0f;
    System.out.println("indise opic scoring updateScore");
    String pageTitle = row.getMetadata().get(new Utf8("title"))!=null ? Bytes.toString(row.getMetadata().get(new Utf8("title")).array()) + "" : "";
    System.out.println(pageTitle);
    System.out.println(inlinkedScoreData.size());
    ...
    System.out.println("score herer : " + oldScore);
  ...
  public void distributeScoreToOutlinks(String fromUrl, WebPage row,
	  ...
	  System.out.println("insdide distributescore");
    ByteBuffer cashRaw = row.getMetadata().get(CASH_KEY);
    if (cashRaw == null) {
      return;
    }
  ...
  public float indexerScore(String url, NutchDocument doc, WebPage row,
      float initScore) {
	  System.out.println("insdide indexerscore");
	  float x = (float) Math.pow(row.getScore(), scorePower) * initScore;
	  System.out.println("printing final score" + x + " ");
	  String pageTitle = row.getMetadata().get(new Utf8("title"))!=null ? Bytes.toString(row.getMetadata().get(new Utf8("title")).array()) + "" : "";
	  System.out.println(pageTitle);
    return x;
  }
