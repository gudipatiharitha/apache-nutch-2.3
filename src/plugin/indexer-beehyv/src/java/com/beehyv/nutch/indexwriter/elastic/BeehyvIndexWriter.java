...
import com.beehyv.holmes.JsonStoreInsertor;
import com.beehyv.holmes.ProductType;
      ...
      while(true) {
        ...
        //this.client.insertElasticSearchDocWithParent("attribute", this.mapper.writeValueAsString(currAttr), parentPageId, "reliance");
         IndexRequestBuilder request =client.prepareIndex(defaultIndex, "attribute")
                .setSource(this.mapper.writeValueAsString(currAttr));//.setParent(parentPageId).get();
        list.add(request);
      }
    }
    ...
    if(sections1 != null) {
      ...
      while(true) {
        ...
        IndexRequestBuilder request = client.prepareIndex(defaultIndex, "section")
                .setSource(this.mapper.writeValueAsString(currSec));//.setParent(parentPageId).get();
        list.add(request);
      }
    }
    return list;
    ...
    for (String fieldName : doc.getFieldNames()) {
       ...
       if (doc.getFieldValues(fieldName).size() > 1) {
        ...
        if(fieldName.equals("json")) {
          ...
          //JsonStoreInsertor insertor = new JsonStoreInsertor();
          Tenant tenant1 = new Tenant(ETenant.AMAZON.getName());
          //insertor.insertEcommercePage(tenant1, ProductType.LAPTOP, "123", content);
         // source.put("jsonsubstr", doc.getFieldValue(fieldName).substring(0,100));
