...
public class Sandhi {
    ...
    static void check(Stack s) {
     ...
     static void check1(Stack s) {
        //clia.unl.unicode.utils.Utils.printOut(Analyser.print, x + "Sandhi");
        byte[] topElmt = ((Entry) s.peek()).getPart();
        byte[] oldTopElmt = topElmt;
        //////System.out.println("INSIDE SNADHI.. hi check1" + UnicodeConverter.revert(topElmt));
        if (ByteMeth.isEqual(topElmt, Constant.athan)
                || ByteMeth.isEqual(topElmt, Constant.ithan)
                || ByteMeth.isEqual(topElmt, Constant.ethan)) {
            euphonic_an(s);
    ...
    static boolean type2(Stack s) {
        ...
        if (topElmt.length >= 2) {
            if (ByteMeth.endsWith(topElmt, Constant.y)) {
                ////System.out.println("value of topElmt in sandhi type 2 " + UnicodeConverter.revert(topElmt));
                if (topElmt[topElmt.length - 2] == Constant.i[0]
                    ...
                    ////System.out.println("value of topElmt in sandhi type 2 inside" + UnicodeConverter.revert(topElmt) + " next " + Constant.ai[0] + " ..... " + topElmt[topElmt.length - 2]);
                    s.pop();
            ...
            } else if (ByteMeth.endsWith(topElmt, Constant.thth)) { // This coding is for to handle the word "அலகாபாத்தின்".
                ////System.out.println("வ்");
                if (topElmt[topElmt.length - 3] == Constant.A[0]) {
                    ...
                    s.push(new Entry(topElmt, -1, oldTopElmt));
                    ////System.out.println("elseType 3 True ");
                    return true;
                }
            }
        }
    ...
    static boolean type4(Stack s) {
        ...
        byte[] topElmt = ((Entry) s.peek()).getPart();
        byte[] oldTopElmt = topElmt;
        ////System.out.println("sandhi in type4:" + UnicodeConverter.revert(topElmt));
        if (BooleanMethod.endsWith_type4_letter(topElmt)) {
            if (!ByteMeth.endsWith(topElmt, Constant.ERk)) { //&& ByteMeth.endsWith(topElmt, Constant.athth)) {
                ////System.out.println("Entering 1 in type4 sandhi");
                //clia.unl.unicode.utils.Utils.printOut(Analyser.print, x + "type 4");
                s.pop();
                topElmt = ByteMeth.addArray(topElmt, Constant.u);
                ////System.out.println("topLemt in type4 sandhi method " + UnicodeConverter.revert(topElmt));
                s.push(new Entry(topElmt, -1, oldTopElmt));

            } else {
                ////System.out.println("Entering 2 in type4 sandhi");
                //  if (!ByteMeth.endsWith(topElmt, Constant.athth)) {
                s.pop();
                topElmt = ByteMeth.subArray(topElmt, 0,
    ...
    static boolean type7(Stack s) {
        // 4 - add u
        byte[] topElmt = ((Entry) s.peek()).getPart();
        byte[] oldTopElmt = topElmt;
        if (BooleanMethod.endsWith_type4_letter(topElmt)) {
            if (!ByteMeth.endsWith(topElmt, Constant.ERk) && (!ByteMeth.endsWith(topElmt, Constant.pp)) && (!ByteMeth.endsWith(topElmt, Constant.p))) {
                //clia.unl.unicode.utils.Utils.printOut(Analyser.print, x + "type 4");
                s.pop();
                topElmt = ByteMeth.addArray(topElmt, Constant.u);
                s.push(new Entry(topElmt, -1, oldTopElmt));
            } else {
                if (ByteMeth.endsWith(topElmt, Constant.pp)) { //this is to handle the words கடிப்பவர், விதைப்பவர்
                    s.pop();
                    topElmt = ByteMeth.subArray(topElmt, 0,
                    ...
                    s.push(new Entry(topElmt, -1, oldTopElmt));
                    return true;
                } else if ((!ByteMeth.endsWith(topElmt, Constant.p))) { // for the word உண்பவன், செய்பவன், .. //update 1-12-12
                    //////System.out.println("Entering 2a in type7 sandhi");
                    s.pop();
                    topElmt = ByteMeth.subArray(topElmt, 0,
                            topElmt.length - Constant.Rk.length);
                    topElmt = ByteMeth.addArray(topElmt, Constant.L);
    ...
    static void kctp(Stack s) {
        ...
        byte[] topElmt = ((Entry) s.peek()).getPart();
        byte[] oldTopElmt = topElmt;
        ////System.out.println("INSIDE SANDHI KCTP "+UnicodeConverter.revert(topElmt));
        if (!ByteMeth.endsWith(topElmt, Constant.Rk)
                ...
                || ByteMeth.endsWith(topElmt, Constant.th)
                || ByteMeth.endsWith(topElmt, Constant.p))) {
            ////System.out.println("INSIDE topelmt in kctp:"+UnicodeConverter.revert(topElmt));
            ...
            ////System.out.println("INSIDE topelmt in kctp: after"+UnicodeConverter.revert(topElmt));
            s.push(new Entry(topElmt, -1, oldTopElmt));
