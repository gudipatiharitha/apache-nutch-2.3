...
public class ADictionary {
	...
	String path = Jumbo.getCLIAHome()+"/resource/unl/ta/Analyser/";
        // String path = "/root/NetBeansProjects/Analyser/Analyser/";
    ...
    public static boolean check(Stack allStk, byte[] topElmt1) {
        ...
        ////System.out.println("value of check in ADictionary "+UnicodeConverter.revert(topElmt1) +"  "+topElmt1.length);
        ...
        for (int i = 0; i < topElmt.length; i++) {
            ////System.out.println("value of check in ADictionary "+UnicodeConverter.revert(topElmt));
            input += topElmt[i];
        }
        ...
        ////System.out.println("chk"+input.length());
    ...
    public static boolean tourismDomain(Stack allStk, byte[] topElmt1) {
        try {
            ...
            if (ByteMeth.endsWith(topElmt, Constant.eyya)) {
                ...
                // adding ஆ  after removing இய from input string

                oldTopElmt = ByteMeth.addArray(oldTopElmt, Constant.A);
                chk = "";
                for (int i = 0; i < oldTopElmt.length; i++) {
                    chk += oldTopElmt[i];
                }
                ...
                oldTopElmt = ByteMeth.subArray(oldTopElmt, 0, oldTopElmt.length - Constant.A.length);  // to remove ஆ to check further rule

                // adding இ with the input string after removing இய from input string

                oldTopElmt = ByteMeth.addArray(oldTopElmt, Constant.i);
                chk = "";
                for (int i = 0; i < oldTopElmt.length; i++) {
                    chk += oldTopElmt[i];
                }
                ...
                oldTopElmt = ByteMeth.subArray(oldTopElmt, 0, oldTopElmt.length - Constant.i.length);  // to remove இ to check further rule

                // adding உ with the input string after removing இய from input string

                oldTopElmt = ByteMeth.addArray(oldTopElmt, Constant.u);
                chk = "";
                for (int i = 0; i < oldTopElmt.length; i++) {
                    chk += oldTopElmt[i];
                }
                ...
                oldTopElmt = ByteMeth.subArray(oldTopElmt, 0, oldTopElmt.length - Constant.u.length);  // to remove உ to check further rule


                oldTopElmt = Sandhi.remove_kctp1(oldTopElmt);  // removing kk, th,pp, ss
                chk = "";
                for (int i = 0; i < oldTopElmt.length; i++) {
                    chk += oldTopElmt[i];
                }
    ...
    public static boolean m_EndNoun(Stack s) {
        ...
        byte[] topElmt = ((Entry) s.peek()).getPart();
        byte[] oldTopElmt = topElmt;
        //System.out.println("Enter into ADictionary m_EndNoun method " + UnicodeConverter.revert(topElmt));
        String input = "";
        for (int i = 0; i < topElmt.length; i++) {
            input += topElmt[i];
        }
    ...
    public static boolean verb(Stack s) {
        //System.out.println("Chking Verb Dic in Adicitonary");
        byte[] topElmt = ((Entry) s.peek()).getPart();
        byte[] oldTopElmt = topElmt;
        String input = "";
        for (int i = 0; i < topElmt.length; i++) {
    ...
    public static boolean Nverb(Stack s) {
        int i = 1;
        int length = 0;
        //System.out.println("Chking Noun+Verb Dic in Adict NVerb");
        byte[] topElmt = ((Entry) s.peek()).getPart();
        byte[] oldTopElmt = topElmt;
        String input = "";
        ...
        if (i != length) {
            ...
            //Sandhi.check(s);
            Case.check(s, true);
            //System.out.println("check Element   after case      " + UnicodeConverter.revert(checkElmt));
            checkElmt = ((Entry) s.peek()).getPart();
            if (Noun_BTree.contains(input)) {
                //System.out.println("Noun");
                s.pop();
